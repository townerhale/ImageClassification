    ...
    case 'colour histogram'
     ...
     case 'bag of sift'
        ...
        if ~exist('vocab.mat', 'file')
            ...
            vocab_size = 300; % you need to test the influence of this parameter
            vocab = build_vocabulary(train_image_paths, vocab_size, COLOUR, 5); %Also allow for different sift parameters
            save('vocab.mat', 'vocab')
        end
        
        % YOU CODE get_bags_of_sifts.m
        if ~exist('image_feats.mat', 'file')
            train_image_feats = get_bags_of_sifts(train_image_paths, COLOUR, 5); %Allow for different sift parameters
            test_image_feats  = get_bags_of_sifts(test_image_paths, COLOUR, 5); 
            save('image_feats.mat', 'train_image_feats', 'test_image_feats')
        else
            image_feats_loaded = load('image_feats.mat');
            train_image_feats = image_feats_loaded.train_image_feats;
            test_image_feats = image_feats_loaded.test_image_feats;
        end
      case 'spatial pyramids'
          ...
          if ~exist('spatial_image_feats.mat', 'file')
            train_image_feats = spatial_pyramid(train_image_paths, COLOUR, 5); %Allow for different sift parameters
            test_image_feats  = spatial_pyramid(test_image_paths, COLOUR, 5); 
            save('spatial_image_feats.mat', 'train_image_feats', 'test_image_feats')
          else
            image_feats_loaded = load('spatial_image_feats.mat');
            train_image_feats = image_feats_loaded.train_image_feats;
            test_image_feats = image_feats_loaded.test_image_feats;
          end                       
          
    case 'cnn'
        if ~exist('cnn_image_feats.mat', 'file')
            [train_image_feats, test_image_feats] = cnn_feature_extraction();
            save('cnn_image_feats.mat', 'train_image_feats', 'test_image_feats')
        else
            image_feats_loaded = load('cnn_image_feats.mat');
            train_image_feats = image_feats_loaded.train_image_feats;
            test_image_feats = image_feats_loaded.test_image_feats;
        end
