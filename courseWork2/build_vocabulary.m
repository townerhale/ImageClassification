...
function vocab = build_vocabulary(image_paths, vocab_size, colour, stepSize)
    ...
    for i=1:size(image_paths)
            ...
            %colour is 1 then use colour information
            if colour == 1
            ...
            %else use grey images
                ...
                img = rgb2gray(img); 
                %get sift features and put in total_d
                [f, total_d] = vl_dsift(single(img), 'fast', 'step', stepSize); 
            end
            siftTotal = [siftTotal, total_d]; 
    end
    %make single
    siftTotal = single(siftTotal);    
    %create clusters from sift features
    [vocab, assignments] = vl_kmeans(siftTotal, vocab_size); 
