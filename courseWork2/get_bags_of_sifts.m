...
function image_feats = get_bags_of_sifts(image_paths, colour, stepSize)
...
% image.
    ...
    %the features for each image will be stored here
    image_feats = zeros(size(image_paths, 1), clusterSize);
    
    %loop through each image
    for i=1:size(image_paths, 1)
        img = imread(image_paths{i}); 
        %get histogram using SIFT features
        histogram = create_sift_features(img, clusterSize, colour, stepSize);
        %put the histogram in image feats
        image_feats(i,:) = transpose(histogram);
    end     
end
